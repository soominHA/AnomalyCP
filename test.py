...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            # path_list.append(items['img_path'])
            # map_list.append(anomaly_map.detach().cpu().numpy())
            # cls_list.append(cls_name)
            # prbs_list.append(text_probs)
    ...
    for obj in tqdm(obj_list):
        ...
        # threshold = image_level_threshold(results, obj)

        # for path, map, cls_name, text_probs in zip(path_list, map_list, cls_list, prbs_list):
        #     visualizer(path, map, args.image_size, args.save_path, cls_name, text_probs, threshold)
