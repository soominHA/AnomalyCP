        ...
        # singl path before "d"
            ...
            if not self.first_layer:
                ...
                if not (counter > len(compound_prompts_deeper) - 1):
                    ...
                    # prefix = x[:1, :, :].to(x.device) # .to(x.device) 추가
                    # suffix = x[1 + self.compound_prompt_nctx:, :, :].to(x.device) # .to(x.device) 추가
                    # textual_context = compound_prompts_deeper[counter].to(x.device) # .to(x.device) 추가
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        # self.conv1 = self.conv1.to(dtype=x.dtype, device=x.device) # 추가
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]

        # class_emb = self.class_embedding.to(dtype=x.dtype, device=x.device) # 추가
        # zeros_tensor = torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device) # 추가
        # x = torch.cat([class_emb + zeros_tensor, x], dim=1)  # shape = [*, grid ** 2 + 1, width] # 추가
        ...
        # pos = self.positional_embedding.to(dtype=x.dtype, device=x.device) # 추가
        pos = self.positional_embedding.to(dtype=x.dtype)
        x = x + pos
        x = self.ln_pre(x)
        ...
        if True:
            ...
            if isinstance(x_ori, list):
                return torch.tensor(x_ori[0])[0, :, :] @ self.proj, patch_tokens
            else:
...
class AnomalyCLIP(nn.Module):
    ...
    def encode_text_learn(self, prompts, tokenized_prompts, deep_compound_prompts_text = None, normalize: bool = False):
        ...
        x = prompts + self.positional_embedding.to(cast_dtype) # 원래
        # x = prompts.to(cast_dtype).to(self.positional_embedding.device) + self.positional_embedding.to(cast_dtype)
